...
export function RecordStage({ record }: { readonly record: UndoRecord }) {
  return (
    <div className="stage-card wide record-card">
      ...
      {record.evidence.length > 0 && <div className="evidence-grid">{record.evidence.map((snapshot) => <article className="evidence-card" key={`${snapshot.offerId}:${snapshot.fingerprint}`}><div className="evidence-header"><h3>{snapshot.merchant}</h3><span>{snapshot.retrievalState === "cached" ? "Cached Evidence" : "Current Evidence"}</span></div><blockquote>“{snapshot.exactText}”</blockquote><dl><div><dt>Scope</dt><dd>{snapshot.scope.kind}: {snapshot.scope.value}</dd></div><div><dt>Collected</dt><dd>{new Date(snapshot.collectedAt).toLocaleString("en-IN", { timeZone: "Asia/Kolkata" })}</dd></div><div><dt>Fingerprint</dt><dd>{snapshot.fingerprint}</dd></div></dl><a href={snapshot.sourceUrl}>{snapshot.sourceUrl} <span aria-hidden="true">↗</span></a></article>)}</div>}
