import { POLICY_FACTS, type EvidenceSnapshot, type Offer, type PolicyAssessment } from "../domain";
...
function entry(options: {
  ...
  return {
    ...
    expected: {
      ...
      remedyWindow: options.window ?? { kind: "unclear" },
      ...
      materialConditions: [],
      supplementaryRemedies: options.defect === "replacement"
        ? [{ kind: "replacement", detail: "Defect replacement", citation: { quote: options.text, sourceUrl } }]
        : [],
      quote: options.text,
      citations,
    },
...
export const FROZEN_POLICY_ANSWER_KEY: ReadonlyArray<PolicyAnswerKeyEntry> = [
  ...
  entry({ id: "missing-deadline-action", offerId: "concept-kart", text: "Returns are accepted for 7 days after delivery. No deadline action, condition, transport, or fee is given.", changeOfMind: "money_back", defect: "none", cost: { kind: "unstated" } }),
  entry({ id: "missing-clock-start", offerId: "concept-kart", text: "A refund request must be submitted within 7 days. The clock start, condition, transport, and fee are not stated.", changeOfMind: "money_back", defect: "none", cost: { kind: "unstated" } }),
  ...
  entry({ id: "fee-silence", offerId: "headphone-zone", text: "Opened unused Products may be refunded within 7 days of delivery after a request and must be self-shipped. Fees are not stated.", changeOfMind: "money_back", defect: "none", window: knownSevenDayWindow, condition: "opened_unused", transport: "self_ship", cost: { kind: "unstated" } }),
